...
fn run_parallel_benchmark(name: &str, log_count: usize, thread_count: Option<usize>) {
    ...
    println!("\n{}", "=".repeat(60));
    println!("📊 Benchmark (Parallel): {}", name);
    ...
    println!("   ✓ Arc<RwLock<>> for thread-safe reads");
    ...
    println!("🔍 Processing logs through radix trie (parallel)...");
    let start = Instant::now();
...
fn benchmark_parallel_scaling() {
    println!("\n{}", "█".repeat(60));
    println!("🚀 PARALLEL SCALING BENCHMARK");
    ...
    println!("{}", "█".repeat(60));
    println!("✅ Parallel scaling benchmark completed!");
    println!("{}", "█".repeat(60));
}
