...
A multi-tenant log ingest service that classifies each record into a stable
template and persists it to ClickHouse. Once logs are templated you can run
distribution-shift analytics over the resulting template counts (Jensen-Shannon
divergence over per-window template histograms) — the typical use case is
"hover a metric chart, see which log shapes correlate with that moment."

Logs arrive over **OTLP/gRPC on port 4317** (any OpenTelemetry collector can
export to it). Templates are generated by an in-memory parse tree at hot-path
speed; novel shapes the parse tree doesn't recognize are sent to a
single-consumer LLM worker that synthesizes a regex template on the fly.
...
End-to-end ingest, single MacBook with local Docker ClickHouse, batch size
1000, concurrency 32:
...
Throughput is what the channel can absorb in burst; sustained durable-write
throughput is bounded by ClickHouse capacity.
...
it. The matched-template rows from the same request flow through
unaffected; the unmatched record is queued and persisted once the consumer
determines its template.
